 ...
 */
class LQTopLepModule: public AnalysisModule {
public:
    
    explicit LQTopLepModule(Context & ctx);
...
LQTopLepModule::LQTopLepModule(Context & ctx){
    // In the constructor, the typical tasks are to initialize the
    // member variables, in particular the AnalysisModules such as
    // CommonModules or some cleaner module, Selections and Hists.
    // But you can do more and e.g. access the configuration, as shown below.
    
    cout << "Hello World from LQTopLepModule!" << endl;
    ...
    dijet_sel.reset(new DijetSelection()); // see LQTopLepSelections

    // 3. Set up Hists classes:
    h_nocuts.reset(new LQTopLepHists(ctx, "NoCuts"));
    h_njet.reset(new LQTopLepHists(ctx, "Njet"));
    h_dijet.reset(new LQTopLepHists(ctx, "Dijet"));
    h_ele.reset(new ElectronHists(ctx, "ele_nocuts"));
}


bool LQTopLepModule::process(Event & event) {
    ...
    cout << "LQTopLepModule: Starting to process event (runid, eventid) = (" << event.run << ", " << event.event << "); weight = " << event.weight << endl;
...
// make sure the LQTopLepModule is found by class name. This is ensured by this macro:
UHH2_REGISTER_ANALYSIS_MODULE(LQTopLepModule)
