...
LQTopLepTriggerHists::LQTopLepTriggerHists(Context & ctx, const string & dirname): Hists(ctx, dirname){
  ...
  book<TH1F>("pt_ele_binned1", "p_{T}^{e} [GeV]", bins_pt.size()-1, &bins_pt[0]);
  ...
  book<TH1F>("eta_ele_binned1", "#eta^{e}", bins_eta.size()-1, &bins_eta[0]); 
  ...
  book<TH1F>("pt_mu_binned1", "p_{T}^{#mu} [GeV]", bins_pt.size()-1, &bins_pt[0]);
  book<TH1F>("eta_mu", "#eta^{#mu}", 24, -2.4, 2.4);
  book<TH1F>("eta_mu_binned", "#eta^{#mu}", 8, -2.4, 2.4);
  book<TH1F>("eta_mu_binned1", "#eta^{#mu}", bins_eta.size()-1, &bins_eta[0]);
