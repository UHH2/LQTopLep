...
void AnalysisTool::ProduceThetaHistograms_mc_fullsyst_much(){
  ...
  vector<TString> syst_shift_theta = {"plus", "minus"};


  TString filename_base = "uhh2.AnalysisModuleRunner.";

  TString outfilename = AnalysisTool::theta_path + "input/theta_histograms_mc_fullsyst_much.root";
      ...
      for(unsigned int k=0; k<systematics.size(); k++){
        ...
        for(unsigned int m=0; m<syst_shift.size(); m++){
          ...
          for(unsigned int i=0; i<samples.size(); i++){
            ...
            if(syst != "nominal") histname_out += "__" + syst + "__" + syst_shift_theta[m];

            TH1F* h_in = (TH1F*)f_in->Get(histname_in);
            if(sample_in.Contains("LQto")){
              // cout << "xsec: " << xsecs[samples_base[i]] << endl;
              h_in->Scale(1./xsecs[samples_base[i]]);
            }
            // cout << "tried to get hist: " << histname_in << endl;
            h_in->SetName(histname_out);
            f_out->cd();
            // cout << "writing hist with name: " << histname_out << endl;
            h_in->Write();
            f_in->Close();
          }
        }
